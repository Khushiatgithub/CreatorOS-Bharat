...
export async function POST(request: NextRequest) {
  try {
    ...
    const newPlan = await SubscriptionPlanModel.create({
      ...
      monthlyPrice: finalBillingCycle === 'monthly' ? finalPrice : (monthlyPrice || Math.round(finalPrice / (12 * 0.8))),
      yearlyPrice: finalBillingCycle === 'yearly' ? finalPrice : (yearlyPrice || Math.round(finalPrice * 12 * 0.8)),
      benefits,
