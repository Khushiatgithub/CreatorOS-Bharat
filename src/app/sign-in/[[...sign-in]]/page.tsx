...
import { Zap, ShieldCheck, ArrowLeft, Mail, Lock, Sparkles, ArrowRight, AlertCircle } from 'lucide-react';
...
function ClerkSignInWrapper() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError(null);
    try {
      // Force Google account chooser screen with prompt: 'select_account'
...
function ResilientDemoSignIn() {
  ...
  const handleEmailSignIn = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      // Check if email matches existing creator
      const cleanEmail = email.trim().toLowerCase();
      const existing = creators.find((c) => c.email?.toLowerCase() === cleanEmail);
      if (existing) {
        ...
        const newCreator = {
          ...
          bio: 'Welcome to my official CreatorOS storefront.',
  ...
  const handleAccountSelect = (account: { name: string; email: string; avatarUrl?: string; isNew?: boolean }) => {
    setShowChooser(false);
    const cleanEmail = account.email.toLowerCase();
    const existing = creators.find(
      (c) => c.email?.toLowerCase() === cleanEmail || c.name.toLowerCase() === account.name.toLowerCase()
    );

    if (existing && !account.isNew) {
      ...
      const newCreator = {
        ...
        avatarUrl: account.avatarUrl || '/avatars/user-avatar.png',
        bannerUrl: 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=1200&q=80',
        verified: false,
        category: 'Digital Creator',
        location: 'India',
  ...
  return (
    <>
      <div className="w-full rounded-[24px] border border-white/[0.12] bg-[#0A0D17]/95 p-7 shadow-2xl backdrop-blur-xl space-y-5 animate-scale-in">
        ...
        {/* Google OAuth Option - Always triggers Google Account Chooser */}
        <button
          onClick={() => setShowChooser(true)}
          className="w-full flex items-center justify-center gap-3 rounded-[14px] border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.09] hover:border-royal-500/40 p-3 text-xs font-semibold text-white transition btn-press group"
          title="Sign in with Google Account Chooser"
        >
          <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
  ...
  return (
    <div className="min-h-screen bg-[#05070B] text-slate-100 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="z-10 w-full max-w-md flex justify-center">
        {isClerk ? <ClerkSignInWrapper /> : <ResilientDemoSignIn />}
      </div>
      ...
      <div className="mt-6 text-center z-10 space-y-3 text-xs text-slate-400 font-mono">
        <div className="flex items-center justify-center gap-2">
          ...
          <span>{isClerk ? 'Secured with Clerk Enterprise Authentication' : 'CreatorOS Resilient Studio Access'}</span>
        </div>
        <div>
