...
export default function StorefrontContent({
  ...
  // Floating WhatsApp CTA calculation
  const rawWhatsapp = creator?.socials?.whatsapp;
  const cleanWhatsappNumber = rawWhatsapp ? rawWhatsapp.replace(/\D/g, '') : null;
  const formattedWhatsappNumber = cleanWhatsappNumber
  ...
  const whatsappMessage = encodeURIComponent(
    "Hi! I found your CreatorOS profile and I'd like to know more about your products and mentorship."
  );
  const whatsappUrl = formattedWhatsappNumber
    ? `https://wa.me/${formattedWhatsappNumber}?text=${whatsappMessage}`
    : null;
  ...
  return (
    <div className={`w-full min-h-screen bg-gradient-to-b ${theme.bgGradient} ${theme.textPrimary} transition-colors duration-500 font-sans`}>
      ...
      <div className={`mx-auto ${isMobilePreview ? 'w-full px-3 py-4' : 'max-w-3xl px-4 py-8 sm:py-12'}`}>
        ...
        {/* ========================================================================= */}
        {/* 3. NAVIGATION TAB BAR (Sticky below navbar) */}
        {/* ========================================================================= */}
        <div className={`sticky ${isMobilePreview ? 'top-0' : 'top-[49px]'} z-30 -mx-4 px-4 py-2.5 mb-8 backdrop-blur-xl bg-[#05070B]/85 transition-all duration-300`}>
          <div id="offerings-section" className="scroll-mt-28 flex items-center gap-1.5 p-1 rounded-[20px] bg-white/[0.04] border border-white/[0.08] overflow-x-auto no-scrollbar shadow-glass-subtle">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-[14px] text-xs font-semibold whitespace-nowrap transition-all duration-200 relative ${
                  activeTab === tab.id
              ...
              <div className="space-y-3.5">
                {products.map((prod) => (
                  <HoverCard hoverY={-3} key={prod.id}>
                    <div 
                      onClick={() => router.push(`/${creator.username}/product/${prod.id}`)}
                      className={`cursor-pointer rounded-[22px] ${theme.cardBg} p-5 transition-all duration-300 border border-white/[0.08] hover:border-royal-500/50 hover:bg-white/[0.04] shadow-glass-card group`}
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                        {/* Cover Thumbnail */}
                        <div className="relative h-28 sm:h-28 sm:w-32 rounded-[16px] overflow-hidden shrink-0 bg-black/40">
                          <img src={prod.coverImage} alt={prod.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <h3 className="font-display text-sm sm:text-base font-bold text-white group-hover:text-royal-300 transition-colors">
                              {prod.title}
                            </h3>
                            <ChevronRight className="h-4 w-4 text-slate-500 group-hover:text-royal-400 group-hover:translate-x-0.5 transition-all shrink-0" />
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-white/[0.08] mt-3">
                            <div className="flex items-baseline gap-2">
                              <span className="font-display text-lg sm:text-xl font-extrabold text-white font-mono">₹{prod.price}</span>
                              {prod.originalPrice && (
                                <span className="text-xs text-slate-500 line-through font-mono">₹{prod.originalPrice}</span>
                              )}
                              <span className="text-[10px] text-emerald-400 font-bold font-mono">
                            ...
                            <RippleButton
                              ...
                              className="rounded-[14px] bg-royal-600 hover:bg-royal-500 px-4 py-2 text-xs font-bold text-white shadow-royal-sm flex items-center gap-1.5 z-10"
                            >
                              <Zap className="h-3.5 w-3.5 fill-white" />
      ...
      {whatsappUrl && (
        <motion.a
          ...
          whileTap={{ scale: 0.95 }}
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-2.5 rounded-full bg-[#25D366] hover:bg-[#20ba59] text-white px-3.5 sm:px-4 py-2.5 sm:py-3 shadow-[0_10px_25px_rgba(37,211,102,0.45)] hover:shadow-[0_14px_30px_rgba(37,211,102,0.6)] border border-emerald-300/30 transition-all duration-300 group`}
          title={`Chat with ${creator.name} on WhatsApp`}
        >
          <div className="relative flex items-center justify-center">
            {/* Pulsing halo */}
            <span className="absolute -inset-1 rounded-full bg-white/30 animate-ping opacity-75 pointer-events-none" />
            <svg className="h-5 w-5 fill-white shrink-0 relative z-10" viewBox="0 0 24 24">
          ...
          <span className="text-xs sm:text-sm font-bold tracking-tight text-white whitespace-nowrap drop-shadow-sm">
            Chat Creator
          </span>
        </motion.a>
      )}
